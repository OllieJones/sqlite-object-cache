...
class WP_Object_Cache extends SQLite3 {
	...
	private function getone( $key, $group ) {
		...
		$stmt->bindValue( ':name', $group . '.' . $key, SQLITE3_TEXT );
		$result = $stmt->execute();
		if ( false === $result ) {
	 ...
	 * @throws Exception Database failure.
	 */
	private function initialize_connection( $tbl = 'object_cache', $noexpire_timestamp_offset = 500000000000 ) {
	...
	public function set( $key, $data, $group = 'default', $expire = 0 ) {
		...
		$former_data = array_key_exists( $group, $this->cache ) && array_key_exists( $key, $this->cache[ $group ] )
